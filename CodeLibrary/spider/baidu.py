...
class BaiDu:
    ...
    def __init__(self, path=None, wait=5, now=False):
        self._path = path
        self.name = '百度'  # 渠道名称
        self.url = 'http://g.baidu.com/a/search.json'
    ...
    async def dispose(self, target, content):
        ...
        for item in data['data']['gameList']:
            text = re.sub(r'<.*?>', '', item.get("name"))
            url = f'http://g.baidu.com/a/{item.get("gameNameShort")}'
