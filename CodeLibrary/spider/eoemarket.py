...
class Eoe:
    ...
    def __init__(self, path=None, wait=5, now=False):
        self._path = path
        self.name = '优亿市场'  # 渠道名称
        self.url = 'http://www.eoemarket.com'
    ...
    def build_param(self, target):
        url = f'{self.url}/search_.html?' + urllib.parse.urlencode([('keyword', target)], encoding='utf-8')
    ...
    async def dispose(self, target, content):
        ...
        with contextlib.suppress(AttributeError):
            ds = soup.find_all("div", {"class": "Rlist"})
            for d in ds:
                a = d.find("a")
                text = re.sub(r'（.*?）', '', a.find("img").get("alt"))
                if cmp(target, text, eq=False):
                    return True, self.url + a.get("href")
        return False, None
