...
class MMY:
    ...
    def __init__(self, path=None, wait=5, now=False):
        self._path = path
        self.name = '木蚂蚁'  # 渠道名称
        self.url = 'http://s.mumayi.com/index.php?'
    ...
    def build_param(self, target):
        url = self.url + urllib.parse.urlencode([('q', target)], encoding='utf-8')
    ...
    async def dispose(self, target, content):
        ...
        with contextlib.suppress(AttributeError):
            ds = soup.find("ul", {"class": "applist"}).find_all("li", {"class": "iapp"})
            for d in ds:
                a = d.find("a")
                text = re.sub(r'（.*?）', '', a.get("alt"))
