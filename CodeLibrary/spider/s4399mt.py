...
class S4399:
    ...
    def __init__(self, path=None, wait=5, now=False):
        self._path = path
        self.name = '4399MT'  # 渠道名称
        self.url = 'http://a.4399.cn'
    ...
    def build_param(self, target):
        url = f'{self.url}/mobile/ajax/search-index.html?' + urllib.parse.urlencode([('key', target)], encoding='utf-8')
        return self.get_param(url)

    async def dispose(self, target, content):
        ds = json.loads(content, encoding='ascii')
        with contextlib.suppress(AttributeError):
            for lk in ds['list']:
                if cmp(target, text=lk['title'], eq=False): return True, f'{self.url}{lk["wap_url"]}'
        return False, None
