...
class MyApp:
    ...
    def __init__(self, path=None, wait=5, now=False):
        self._path = path
        self.name = '应用宝'  # 渠道名称
        self.url = 'https://android.myapp.com/myapp'
    ...
    def build_param(self, target):
        url = f'{self.url}/searchAjax.htm?' + urllib.parse.urlencode([('kw', target)], encoding='utf-8')
        return self.get_param(url)

    async def dispose(self, target, content):
        ds = json.loads(content, encoding='utf-8')
        with contextlib.suppress(AttributeError):
            for item in ds['obj']['items']:
                text = item['appDetail']['appName']
                url = f'{self.url}/detail.htm?apkName={item["pkgName"]}'
                if cmp(target, text, eq=False): return True, url
        return False, None
