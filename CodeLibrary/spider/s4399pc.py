...
class S4399:
    ...
    def __init__(self, path=None, wait=5, now=False):
        self._path = path
        self.name = '4399PC'  # 渠道名称
        self.url = 'http://so2.4399.com/search/search.php?'
    ...
    def build_param(self, target):
        url = self.url + urllib.parse.urlencode([('k', target)], encoding='gb2312')
        return self.get_param(url)

    async def dispose(self, target, content):
        soup = BeautifulSoup(content.decode(encoding='gb18030'), features='html.parser')
        with contextlib.suppress(AttributeError):
            als = soup.find_all("a", {"class": "fl_img"})
            for a in als:
                text = a.find("img").get("alt")
                if cmp(target, text, eq=False): return True, a.get("href")
        return False, None
