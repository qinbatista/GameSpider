...
class SoGou:
    ...
    def build_param(self, target):
        url = f'{self.url}/ajax/search/gamesearch.do?' + urllib.parse.urlencode([('q', target)], encoding='utf-8')
        return self.get_param(url)

    async def dispose(self, target, content):
        ds = json.loads(content, encoding='utf-8')
        with contextlib.suppress(AttributeError):
            for item in ds['result']['games']:
                text = re.sub(r'<.*?>', '', item['name'])
                url = f'{self.url}/{item["gid"]}'
