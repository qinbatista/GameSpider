...
class HuaWei:
    ...
    def __init__(self, path=None, wait=5, now=False):
        self._path = path
        self.name = '华为'  # 渠道名称
        self.url = 'http://appstore.huawei.com'
    ...
    def build_param(self, target):
        url = f'{self.url}/search/' + urllib.parse.quote(target, encoding='utf-8')
    ...
    async def dispose(self, target, content):
        ...
        with contextlib.suppress(AttributeError):
            divs = soup.find_all("div", {"class": "game-info-ico"})
            for div in divs:
                a = div.find("a", {'title': True})
