...
class S360:
    ...
    def __init__(self, path=None, wait=5, now=False):
        self._path = path
        self.name = '360'  # 渠道名称
        self.url = 'http://u.360.cn/search/search/?'
    ...
    def build_param(self, target):
        url = self.url + urllib.parse.urlencode([('key_word', target)], encoding='utf-8')
    ...
    async def dispose(self, target, content):
        ...
        with contextlib.suppress(AttributeError):
            lis = soup.find("ul", {"class": "serlists"}).find_all("li")
            for li in lis:
                a = li.find('strong').find('a', {'target': True})
                text, url = a.text, a.get("href")
