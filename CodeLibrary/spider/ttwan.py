...
class TTW:
    ...
    def __init__(self, path=None, wait=5, now=False):
        self._path = path
        self.name = 'TT玩'  # 渠道名称
        self.url = 'http://www.tzshouyou.com/index.php?'
    ...
    def build_param(self, target):
        url = self.url + urllib.parse.urlencode([('ac', 'search'), ('keyword', target)], encoding='utf-8')
    ...
    async def dispose(self, target, content):
        ...
        with contextlib.suppress(AttributeError):
            ds = soup.find_all("a", {"class": "dg_icon_Search"})
            for a in ds:
                text = re.sub(r'（.*?）', '', a.find("img").get("alt"))
                if cmp(target, text, eq=False):
                    return True, a.get("href")
        return False, None
