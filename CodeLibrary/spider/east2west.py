...
class E2W:
    ...
    def __init__(self, path=None, wait=5, now=False):
        self._path = path
        self.name = '东品游戏'  # 渠道名称
        self.url = 'http://www.gamer-gamer.com/gamergamer/index.php?g=portal&m=search&a=index'
    ...
    def build_param(self, target):
        url = self.url
        data = {'keyword': target}
        return self.get_param(url, method='post', data=data)

    @staticmethod
    async def dispose(target, content):
        soup = BeautifulSoup(content.decode(encoding='utf-8'), features='html.parser')
        with contextlib.suppress(AttributeError):
            divs = soup.find('div', {'class': 'span9'}).find_all('div', {'class': 'tc-gridbox'})
            for div in divs:
                a = div.find('a')
                text = a.find('img').get('alt')
                if cmp(target, text): return True, f'http://www.gamer-gamer.com{a.get("href")}'
        return False, None
