...
class MZWan:
    ...
    def __init__(self, path=None, wait=5, now=False):
        self._path = path
        self.name = '拇指玩'  # 渠道名称
        self.url = 'https://www.muzhiwan.com'
    ...
    def build_param(self, target):
        url = f'{self.url}/search.html?' + urllib.parse.urlencode([('q', target)], encoding='utf-8')
    ...
    async def dispose(self, target, content):
        ...
        if soup.find('div', {'class': 'nosearch'}):
            return False, False
        with contextlib.suppress(AttributeError):
            als = soup.find_all("a", {"class": "list-icon"})
            for a in als:
                if cmp(target, text=a.get('title')):
