...
class Gionee:
    ...
    def __init__(self, path=None, wait=5, now=False):
        self._path = path
        self.name = '金立'  # 渠道名称
        self.url = 'http://game.gionee.com/Front/Search/index/?'
    ...
    def build_param(self, target):
        url = self.url + urllib.parse.urlencode([('keyword', target), ('action', 'search'), ('object', 'searchlist'),
                                                 ('intersrc', 'homepage')], encoding='utf-8')
    ...
    async def dispose(self, target, content):
        ...
        lis = soup.find("ul", {"class": "search_list"}).find_all("li")
        for li in lis:
            a = li.find("h4").find("a")
            text, url = a.get("title"), a.get("href")
            if cmp(target, text): return True, url
        return False, None
