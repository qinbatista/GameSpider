...
class WDJ:
    ...
    def __init__(self, path=None, wait=5, now=False):
        self._path = path
        self.name = '豌豆荚'  # 渠道名称
        self.url = 'https://www.wandoujia.com/search?'
    ...
    def build_param(self, target):
        url = self.url + urllib.parse.urlencode([('key', target)], encoding='utf-8')
    ...
    async def dispose(self, target, content):
        ...
        with contextlib.suppress(AttributeError):
            ...
            lis = ul.find_all("li", {"class": "search-item search-searchitems"})
            for li in lis:
                a = li.find("h2", {"class": "app-title-h2"}).find("a")
                if cmp(target, text=a.text): return True, a.get("href")
        return False, None
