...
class TouTiao:
    ...
    def __init__(self, path=None, wait=5, now=False):
        self._path = path
        self.name = '今日头条'  # 渠道名称
        self.url = 'https://ic.snssdk.com/game_channel/api/search_content?'
    ...
    def build_param(self, target):
        url = self.url + urllib.parse.urlencode([('keyword', target)], encoding='utf-8')
        return self.get_param(url)

    async def dispose(self, target, content):
        data = json.loads(content.decode(encoding='utf-8'))
        for item in data['data'].get('game_list'):
            text = item.get("name")
            url = item.get("download_info").get("download_url")
