...
class KKPao:
    ...
    def __init__(self, path=None, wait=5, now=False):
        self._path = path
        self.name = '酷酷跑'  # 渠道名称
        self.url = 'https://www.kukupao.com/category/p1/'
    ...
    def build_param(self, target):
        url = self.url + urllib.parse.quote(target, encoding='utf-8')
    ...
    async def dispose(self, target, content):
        ...
        with contextlib.suppress(AttributeError):
            divs = soup.find("div", {"id": "search-result"}).find_all("div", {"class": "game cf"})
            for div in divs:
                a = div.find("a", {'class': 'logo'})
                text = a.get("title")
                if cmp(target, text): return True, a.get("href")
        return False, None
